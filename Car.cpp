...
int main(int argc, char*argv[])
{
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        vec3 CarDir = vec3(cosf(radians(-(c.get_Orientation()))), -0.5f,sinf(radians(-(c.get_Orientation()))));
       
        
        //USE ENUM BTW THREE VIEW VALUES
        ...
        else if (view == carView::first)
        {
            ...
            mat4 viewMatrix = lookAt(c.GetThirdPersonPosition(),c.GetThirdPersonPosition() + CarDir, cameraUp);
            ...
            mat4 viewMatrix = lookAt(c.GetFirstPersonPosition(),c.GetFirstPersonPosition() + CarDir, cameraUp);
            setViewMatrix(shaderProgram, viewMatrix);
            setViewMatrix(textShaderProgram, viewMatrix);
            setViewMatrix(lightShaderProgram, viewMatrix);
            setViewMatrix(gridShaderProgram, viewMatrix);
        ...
        c.DrawGrid(simpleDepthShaderProgram);
        glBindVertexArray(vao_cube);
        c.DrawBody(simpleDepthShaderProgram);
        c.DrawBonnet(simpleDepthShaderProgram);
        c.DrawRoof(simpleDepthShaderProgram);
        c.DrawTrunk(simpleDepthShaderProgram);
        c.DrawHeadLights(simpleDepthShaderProgram);
        glBindVertexArray(vao_circle);
        c.DrawWheels2(simpleDepthShaderProgram, angle);
        //glBindVertexArray(0);
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        glCullFace(GL_BACK);
        ...
        if (gridTexture == false){
            ...
            c.DrawGrid_x(gridShaderProgram);
            
            glBindVertexArray(vao_grid_z);
            c.DrawGrid_z(gridShaderProgram);
            ...
            glBindVertexArray(vao_grid);
            c.DrawGrid(gridTextShaderProgram);
        ...
        //Draw car's parts
        //glUseProgram(simpleDepthShaderProgram);
        c.DrawBody(carshader);
        c.DrawBonnet(carshader);
        c.DrawRoof(carshader);
        c.DrawTrunk(carshader);
        ...
        //Drawing HeadLights
        c.DrawHeadLights(lightShaderProgram);
        ...
        if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS){
            ...
            int xpos=c.get_xPos();
            xpos+=(dt * 55);
            c.SetPositions(xpos, c.get_yPos(), c.get_zPos());
            leftLightPos = c.GetRightHeadLight();
            rightLightPos = c.GetLeftHeadLight();
            leftTailightPos = c.GetLeftTailight();
            rightTailightPos = c.GetRightTailight();
        }
        ...
        if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS){
            ...
            int zpos=c.get_zPos();
            zpos+=(dt * 55) + 1;
            c.SetPositions(c.get_xPos(), c.get_yPos(), zpos);
            leftLightPos = c.GetRightHeadLight();
            rightLightPos = c.GetLeftHeadLight();
            leftTailightPos = c.GetLeftTailight();
            rightTailightPos = c.GetRightTailight();
        ...
        if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS){
            ...
            float orientangle=c.get_Orientation();
            orientangle+=dt * 20;
            c.SetOrientationAngle(orientangle);
            leftLightPos = c.GetRightHeadLight();
            rightLightPos = c.GetLeftHeadLight();
            leftTailightPos = c.GetLeftTailight();
            rightTailightPos = c.GetRightTailight();
        ...
        if (glfwGetKey(window, GLFW_KEY_M) == GLFW_PRESS){
            ...
            float turnangle=c.get_Turn();
            
            turnangle+=dt * 30;
            
            c.SetTurnAngle(turnangle);
        ...
        if (glfwGetKey(window, GLFW_KEY_O) == GLFW_PRESS){
            ...
            float orientation = c.get_Orientation();
            float turnangle = c.get_Turn();
            
            if (turnangle==0){
                float dirx = cosf(radians(orientation));
                float dirz = -(sinf(radians(orientation)));
                ...
                c.SetPositions(xpos, c.get_yPos(), zpos);
            }
                ...
                c.SetOrientationAngle(orientation);
