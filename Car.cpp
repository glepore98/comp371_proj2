...
int main(int argc, char*argv[])
{
    ...
    //Class objects
    CarParts c[6] = {{0,0,10,10.0},{4,0,-5,0.0},{8,0,5,-30.0},{12,0,0,0.0},{16,0,-5,20.0},{-16,0,10,60.0}};
    //vector<CarParts> c;
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        //Sets each car's bounded box for each of its components
        for(int i=0; i<6; i++){
            c[i].SetBoxParts();
        }
            
        //Checks collisions for all cars on grid
        for (int i =0; i<6; i++){
           for (int j =0; j<6; j++){
               if (i!=j){
                   if (((c[i].get_xPos() - c[j].get_xPos()) < 2) && ((c[i].get_zPos() - c[j].get_zPos())< 2)){
                       col.CheckCollision(c[i], c[j]);
                   }
                }
            }
        }
        ...
        //c[0].SetOrientationAngle(45);
        //c[0].setFront(vec3(0.707,0.0,0.707));
        //cout<<c[0].getFront().x<<endl;
        ...
        if (glfwGetKey(window, GLFW_KEY_U) == GLFW_PRESS){
            ...
            for (int i=0; i<6;i++){
            c[i].SetScale(scale);
            leftLightPos = c[i].GetRightHeadLight();
            rightLightPos = c[i].GetLeftHeadLight();
            leftTailightPos = c[i].GetLeftTailight();
            rightTailightPos = c[i].GetRightTailight();
            }
        }
